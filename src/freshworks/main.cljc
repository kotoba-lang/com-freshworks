(ns freshworks.main
  "Kotodama WASM entrypoint for the Freshworks clean-room actor (L4) — Clojure port.
  ...
  facts (`freshworks.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "freshworks")
(def tier "L4")
...
(def entity-specs
  [{:entity "Ticket"       :plural "tickets"       :id-prefix "freshwor_tic"
    :fields [:subject :requesterId :priority :status]       :required [:subject :priority]
    :coerce {}                                  :refs {}}
   {:entity "Survey"       :plural "surveys"       :id-prefix "freshwor_sur"
    :fields [:title :type :status]                          :required [:title :type]
    :coerce {}                                  :refs {}}
   {:entity "Response"     :plural "responses"     :id-prefix "freshwor_res"
    :fields [:surveyId :respondentId :score :submittedAt]   :required [:score :submittedAt]
    :coerce {:score :int}                       :refs {:surveyId "Survey"}}
   {:entity "Account"      :plural "accounts"      :id-prefix "freshwor_acc"
    :fields [:name :healthScore :arr]                       :required [:name :healthScore]
    :coerce {:healthScore :float :arr :float}   :refs {}}
   {:entity "Conversation" :plural "conversations" :id-prefix "freshwor_con"
    :fields [:accountId :channel :sentiment]                :required [:channel :sentiment]
    :coerce {:sentiment :float}                 :refs {:accountId "Account"}}
   {:entity "Contact"      :plural "contacts"      :id-prefix "freshwor_con"
    :fields [:email :firstName :lastName]                   :required [:email :firstName]
    :coerce {}                                  :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"freshworks.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "freshworks-compat" :tier tier :entities entities} 200])
...
(defn start! [] :freshworks-compat/ready)
